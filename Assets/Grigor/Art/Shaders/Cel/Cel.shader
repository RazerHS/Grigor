...
Shader "GRIGOR/Cel"
{
	Properties
	{
		...
		_BandAmount("Band Amount", Range( 1 , 5)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput inputMesh  )
			{
				...
				inputMesh.normalOS = UnpackNormalScale( tex2Dlod( _NormalMap, float4( uv_NormalMap, 0, 0.0) ), 1.0f );
					...
					)
			{
				...
				float dotResult13 = dot( _DirectionalLightDatas[0].forward , packedInput.ase_normal );
				float temp_output_18_0 = ( ( dotResult13 * 0.5 ) + 0.5 );
				float manualBand51 = ( floor( ( temp_output_18_0 * _BandAmount ) ) * ( 1.0 / _BandAmount ) );
				
				float2 uv_EmissiveMap = packedInput.ase_texcoord1.xy * _EmissiveMap_ST.xy + _EmissiveMap_ST.zw;
				float4 color38 = IsGammaSpace() ? float4(1,1,1,0) : float4(1,1,1,0);
				
				surfaceDescription.Color = ( tex2DNode36 * manualBand51 ).rgb;
				surfaceDescription.Emission = ( tex2D( _EmissiveMap, uv_EmissiveMap ) * color38 ).rgb;
				surfaceDescription.Alpha = tex2DNode36.a;
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput  ) : SV_Target
			{
				...
				float2 uv_EmissiveMap = packedInput.ase_texcoord2.xy * _EmissiveMap_ST.xy + _EmissiveMap_ST.zw;
				float4 color38 = IsGammaSpace() ? float4(1,1,1,0) : float4(1,1,1,0);
...
ASEEND*/
//CHKSM=054FC0188F844D3B6BF79D0E507F8D69293DE453