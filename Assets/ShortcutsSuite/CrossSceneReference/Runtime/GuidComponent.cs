...
public class GuidComponent : MonoBehaviour, ISerializationCallbackReceiver 
{
    ...
    void CreateGuid()
    {
        ...
        if (serializedGuid == null || serializedGuid.Length != 16)
        {
	        ...
	        bool isPartOfPrefabInstance = PrefabUtility.IsPartOfPrefabInstance(this);
            PrefabInstanceStatus prefabStatus = PrefabUtility.GetPrefabInstanceStatus(this);
            if (isPartOfPrefabInstance && prefabStatus != PrefabInstanceStatus.Connected)
            {
...
#if UNITY_2018_3_OR_NEWER
		bool isPrefab = gameObject?.scene.name == null ? true : false;
	    bool isPartOfPrefabInstance = PrefabUtility.IsPartOfPrefabInstance(this);
	    if (isPrefab && !isPartOfPrefabInstance)
        {
			serializedGuid = null;
            guid = System.Guid.Empty;
        }
