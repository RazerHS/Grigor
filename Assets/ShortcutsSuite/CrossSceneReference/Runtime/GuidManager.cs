...
public class GuidManager
{
    ...
    private bool InternalAdd(GuidComponent guidComponent)
    {
        ...
        if ( existingInfo.go != null && existingInfo.go != guidComponent.gameObject )
        {
            ...
            {
                Debug.AssertFormat(false, guidComponent, "Guid Collision Detected between {0} and {1}.\nAssigning new Guid. Consider tracking runtime instances using a direct reference or other method.", (guidToObjectMap[guid].go != null ? guidToObjectMap[guid].go.scene.name + guidToObjectMap[guid].go.name : "NULL"), (guidComponent != null ? guidComponent.gameObject.scene.name + guidComponent.name : "NULL"));
            }
            else
            {
                ...
                Debug.LogWarningFormat(guidComponent, "Guid Collision Detected while creating {0}. Clashes with {1}\nAssigning new Guid.", (guidComponent != null ? guidComponent.gameObject.scene.name + " " + guidComponent.name : "NULL"), existingInfo.go.scene.name + " " + existingInfo.go.name);
            }
            return false;
        }
